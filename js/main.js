<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Trust Bit - Cryptocurrency Exchange</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <header class="header">
        <div class="logo">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
            </svg>
            Trust Bit
        </div>

        <nav class="nav">
            <a href="index.html" data-page="home" class="active">Home</a>
            <a href="html/trading.html" data-page="trading">Trade</a>
            <a href="html/wallet.html" data-page="wallet">Wallet</a>
            <a href="html/portfolio.html" data-page="portfolio">Portfolio</a>
            <a href="html/history.html" data-page="history">History</a>
            <a href="html/kyc.html" data-page="kyc">Profile</a>
        </nav>

        <div class="user-controls">
            <div class="profile-dropdown" id="profile-dropdown">
                <button class="profile-btn" id="profile-btn" onclick="window.location.href='html/kyc.html'">Profile</button>
                <div class="dropdown-menu" id="dropdown-menu">
                    <a href="html/kyc.html" data-page="kyc">Profile</a>
                    <a href="#" onclick="logout()">Logout</a>
                </div>
            </div>
            <button class="login-btn" id="login-btn">Login</button>
            <button class="register-btn" id="register-btn">Register</button>
        </div>
    </header>

    <div class="modal" id="login-modal">
        <div class="modal-content">
            <span class="modal-close" onclick="closeModal('login-modal')">×</span>
            <h2>Login</h2>
            <div class="input-group">
                <label for="login-username">Username</label>
                <input type="text" id="login-username" placeholder="Enter username">
            </div>
            <div class="input-group">
                <label for="login-password">Password</label>
                <input type="password" id="login-password" placeholder="Enter password">
            </div>
            <p id="login-error">Invalid username or password</p>
            <button class="btn btn-primary" id="login-submit">Login</button>
        </div>
    </div>

    <div class="modal" id="register-modal">
        <div class="modal-content">
            <span class="modal-close" onclick="closeModal('register-modal')">×</span>
            <h2>Register</h2>
            <div class="input-group">
                <label for="register-username">Username</label>
                <input type="text" id="register-username" placeholder="Enter username">
            </div>
            <div class="input-group">
                <label for="register-password">Password</label>
                <input type="password" id="register-password" placeholder="Enter password">
            </div>
            <div class="input-group">
                <label for="register-email">Email</label>
                <input type="email" id="register-email" placeholder="Enter email">
            </div>
            <p id="register-error">Please check your input</p>
            <button class="btn btn-primary" id="register-submit">Register</button>
        </div>
    </div>

    <main class="main-content active" id="home">
        <section class="hero-section">
            <div class="hero-text">
                <h1>Trade with Trust Bit</h1>
                <p>A reliable and secure platform for cryptocurrency trading. Start your journey in the world of digital assets.</p>
            </div>
        </section>

        <section class="promotions">
            <div class="promo-card">
                <h3>0% Fees</h3>
                <p>Trade with zero fees on the spot market until the end of the month!</p>
                <button class="btn-primary" onclick="restrictRegion(event)">Learn More</button>
            </div>
            <div class="promo-card">
                <h3>Staking</h3>
                <p>Earn up to 8% annually on ETH and USDT staking.</p>
                <button class="btn-primary" onclick="restrictRegion(event)">Start Staking</button>
            </div>
            <div class="promo-card">
                <h3>Referral Program</h3>
                <p>Invite friends and earn 20% of their trading fees.</p>
                <button class="btn-primary" onclick="restrictRegion(event)">Invite a Friend</button>
            </div>
        </section>

        <section class="news-section">
            <h2>Cryptocurrency News</h2>
            <div class="news-list" id="news-list"></div>
        </section>
    </main>

    <button class="support-button" onclick="toggleSupportChat()">💬</button>
    <div class="support-chat" id="support-chat">
        <div class="chat-header">
            <h3>Trust Bit Support</h3>
            <button class="chat-close" onclick="toggleSupportChat()">✕</button>
        </div>
        <div class="chat-body" id="chat-body">
            <div class="chat-message support">Welcome! How can I assist you?</div>
        </div>
        <div class="chat-input">
            <input type="text" id="chat-input" placeholder="Type your message...">
            <button onclick="sendMessage()">Send</button>
        </div>
    </div>

    <footer class="custom-footer">
        <div class="footer-section">
            <h3>Trust Bit</h3>
            <a href="#" onclick="restrictAccess(event)">About Us</a>
            <a href="#" onclick="restrictAccess(event)">Team</a>
            <a href="#" onclick="restrictAccess(event)">Support</a>
        </div>
        <div class="footer-section">
            <h3>Features</h3>
            <a href="#" onclick="restrictAccess(event)">Technical Data</a>
            <a href="#" onclick="restrictAccess(event)">Security</a>
            <a href="#" onclick="restrictAccess(event)">Performance</a>
        </div>
        <div class="footer-section">
            <h3>Market Tools</h3>
            <a href="#" onclick="restrictAccess(event)">Analytics</a>
            <a href="#" onclick="restrictAccess(event)">Indicators</a>
            <a href="#" onclick="restrictAccess(event)">Trading Signals</a>
        </div>
        <div class="footer-section">
            <h3>Legal Information</h3>
            <a href="#" onclick="restrictAccess(event)">Privacy Policy</a>
            <a href="#" onclick="restrictAccess(event)">Terms of Use</a>
            <a href="#" onclick="restrictAccess(event)">Licenses</a>
        </div>
        <div class="footer-section">
            <h3>Trading Pairs</h3>
            <a href="#" onclick="restrictAccess(event)">BTC/USD</a>
            <a href="#" onclick="restrictAccess(event)">ETH/USDT</a>
            <a href="#" onclick="restrictAccess(event)">ADA/USDT</a>
        </div>
    </footer>

    <script>
        // Restrict access to footer links
        function restrictAccess(event) {
            event.preventDefault();
            alert('Not available in your region.');
        }
    </script>
    <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
    <script src="https://unpkg.com/lightweight-charts/dist/lightweight-charts.standalone.production.js"></script>
    <script type="module" src="js/main.js"></script>
</body>
</html>
